// Network Flow Algorithm using Capacity Scaling and DFS
// Author : Samin Yasar
...
protected:
    ...
    int visitedMarker;              // Marker for visited nodes
private:
    long long maxCapacity;          // Edge with maximum capacity of the graph
...
public:
    ...
    void addEdge(int from, int to, long long capacity) // Adding an edge and its residual edge to the graph
    {
        ...
        if (maxCapacity < capacity) // If the currently added edge has more sapacity than the max capacity then update the max capacity
        {
...
protected:
    void markAllNodesUnvisited()         // Sets all nodes unvisited by changing the visited marker
    {
        visitedMarker++;
    }
    bool isVisited(int node) const       // Whether the node is visited or not
    {
        return visited[node] == visitedMarker;
    }
    void setVisited(int i)               // Marks node i as visited
    {
        visited[i] = visitedMarker;
    }
    long long getMaxCapacity() const     // Returns the maximum edge capacity of the graph 
    {
        return maxCapacity;
    }
...
class CapacityScalingDFS : public NetworkFlowSolver
{
        long long scalingFactor;     // Scaling factor to find the minimum capacity of an edge that can be added to the augmenting path
    public:
        CapacityScalingDFS(int n, int s, int t) : NetworkFlowSolver(n, s, t) {} // Constructor to initialize the graph and other properties
    private:
        long long maxCapacity = getMaxCapacity(); // Maximum capacity of the graph
        void findScalingFactor()                  // Find the scaling factor by multiplying 2 until it is less than the maximum edge capacity of the graph
        {
            scalingFactor = 1;
        ...
        void solve() override                     // Overriding the solve method to implement Capacity Scaling algorithm
        {
            findScalingFactor();
            while (scalingFactor > 0)
            {
                for (long long f = dfs(s, INF); f != 0; f = dfs(s, INF)) // Runs the algorithm until the bottleneck value not equals to zero that means there is lack of capacity
                {
                ...
                scalingFactor /= 2;               // Decrease the scaling factor by half if there is no augmenting path with the current scaling factor
            }
        }
    ...
    private:
        ...
        {
            if (node == t)               // If sink is found return the flow
            {
                return flow;
            }
            setVisited(node);            // Mark the node as visited
            auto &edges = adjList[node]; // Get the edges of the node
            for (Edge *edge : edges)     // For each out edges from the node get remaining capacity
            {
                long long remainedCapacity = edge->remainingCapacity(); // and if we have remaining capacity and the to node of the edge
                if (remainedCapacity > 0 && !isVisited(edge->to))       // is not yet in the augmenting path then go for the next check
                {
                    if (remainedCapacity >= scalingFactor) // If the remaining capacity is greater than or equal to scaling factor then do dfs
                    {
                        long long bottleneck = dfs(edge->to, min(flow, remainedCapacity)); // Recursively call dfs to get the bottleneck value
                        if (bottleneck > 0)                                                // If we get a valid bottleneck augment each edge
                        {
                            edge->augment(bottleneck); // Augment the edge by adding bottleneck value to the flow
...
public:
    static void exampleGraph1() // Example 1 :
    {
        ...
        int s = n - 1;
        int t = n - 2;
        CapacityScalingDFS solver1(n, s, t);
    ...
    static void exampleGraph2() // Example 2 :
    {
        ...
        CapacityScalingDFS solver2(n, s, t);
    ...
    static void exampleGraph3() // Example 3 :
    {
        ...
        CapacityScalingDFS solver3(n, s, t);
    ...
    static void exampleGraph4() // Example 4 :
    {
        ...
        CapacityScalingDFS solver4(n, s, t);
...
int main()
{
    CapacityScalingDFS::exampleGraph1();
    CapacityScalingDFS::exampleGraph2();
    CapacityScalingDFS::exampleGraph3();
    CapacityScalingDFS::exampleGraph4();
    return 0;
} 
